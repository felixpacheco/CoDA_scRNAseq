...
# RAW DATA
set.seed(42)
ran <- sample(1:nrow(bulk_counts), 0.5 * nrow(bulk_counts_t)) 
 
##extract training set
bulk_counts <- bulk_counts[,!names(bulk_counts) %in% "Ensembl_gene_id"]
...
## extract labels
targets_train<- str_sub(colnames(bulk_counts_t), start=5,end=5)[ran]
targets_test <-  str_sub(colnames(bulk_counts_t), start=5,end=5)[-ran]
...
set.seed(42)
ran <- sample(1:nrow(deseqcounts), 0.8 * nrow(deseqcounts_t)) 
 
##extract training set
deseqcounts <- deseqcounts[,!names(deseqcounts) %in% "Ensembl_gene_id"]
deseqcounts_train <- deseqcounts[ran,] 

##extract testing set
deseqcounts_test <- deseqcounts[-ran,] 

## extract labels
targets_train<- str_sub(colnames(deseqcounts_t), start=5,end=5)[ran]
targets_test <-  str_sub(colnames(deseqcounts_t), start=5,end=5)[-ran]
# Compute KNN function
library(class)
...
for (i in seq_along(k_to_try)) {
  pred = knn(train = deseqcounts_train, 
             test  = deseqcounts_test, 
             cl    = targets_train, 
             k     = k_to_try[i])
  tab = table(pred, targets_test)
...
set.seed(42)
ran <- sample(1:nrow(edge_counts), 0.8 * nrow(edge_counts_t)) 
 
##extract training set
edge_counts <- edge_counts[,!names(edge_counts) %in% "Ensembl_gene_id"]
edge_counts_train <- edge_counts[ran,] 

##extract testing set
edge_counts_test <- edge_counts[-ran,] 

## extract labels
targets_train<- str_sub(colnames(edge_counts_t), start=5,end=5)[ran]
targets_test <-  str_sub(colnames(edge_counts_t), start=5,end=5)[-ran]
# Compute KNN function
library(class)
...
set.seed(42)
ran <- sample(1:nrow(aldex_counts), 0.8 * nrow(aldex_counts_t)) 
 
##extract training set
aldexcounts <- aldex_counts[,!names(aldex_counts) %in% "Ensembl_gene_id"]
aldex_counts_train <- aldex_counts[ran,] 

##extract testing set
aldex_counts_test <- aldex_counts[-ran,] 

## extract labels
targets_train<- str_sub(colnames(aldex_counts_t), start=5,end=5)[ran]
targets_test <-  str_sub(colnames(aldex_counts_t), start=5,end=5)[-ran]
# Compute KNN function
library(class)
